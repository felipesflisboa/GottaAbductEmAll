...
namespace game {
    ...
    export class BackgroundShipSystem extends ut.ComponentSystem {
        OnUpdate():void {
            this.world.forEach(
                [ut.Entity, game.BackgroundShip, ut.Core2D.TransformLocalPosition], 
                (entity, ship, tLocalPos) => {
                    if(ship.speed==0)
                        this.Initialize(entity, tLocalPos);
                    let changePosition = false;
                    if(ship.way)
                        changePosition = tLocalPos.position.x > X_LIMIT;
                    else
                        changePosition = tLocalPos.position.x < -X_LIMIT;
                    if(changePosition){
                    ...
                    tLocalPos.position = tLocalPos.position.add(new Vector3(
                        ship.speed*(ship.way ? 1 : -1)*this.scheduler.deltaTime(), 0, 0
                    ));
                }
            );
        }
